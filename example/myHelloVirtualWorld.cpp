...
struct Agent : public SoundSource, public Nav, public Drawable {
  Agent() : oscPhase(0), oscEnv(1) { Nav::pos(-4, 0, -4); }

  virtual ~Agent() {}

  virtual void onProcess(AudioIOData& io) {
    static double gain = 0;
    static bool lookingForZeroCrossing = true;
    static double _s = 0;

    while (io()) {
      float s = sin(oscPhase * M_2PI);
      if (lookingForZeroCrossing)
        if (s * _s < 0) {
          lookingForZeroCrossing = false;
          gain = 1;
        }

      if (!lookingForZeroCrossing)
        if (gain < 0.001) {
          lookingForZeroCrossing = true;
          gain = 0;
        }
...
int main(int argc, char* argv[]) {
  ...
  scene.usePerSampleProcessing(
      true);  // per sample processing is less efficient than per buffer
              // (default), but provides higher quality Doppler
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    windows[i].transform.quat().fromAxisAngle(M_PI / 2 - i * M_PI / 2,
                                              Vec3d(0, 1, 0));
  ...
  for (int i = 4; i < 6; ++i) {
    windows[i].append(*new StandardWindowKeyControls);
    windows[i].append(*new NavInputControl(navMaster));
    windows[i].transform.quat().fromAxisAngle(M_PI / 2 - i * M_PI,
                                              Vec3d(1, 0, 0));
    windows[i].cam.fovy(90);
  }
